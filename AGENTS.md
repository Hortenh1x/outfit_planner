...
- The paywall is enforced (stages 1–4 of `PAYWALL_MODEL.md`): one Application `PlanCatalog` (role → `PlanLimits`; numeric caps/allowances overridable via `Paywall__Free__*`/`Paywall__Premium__*`, zero/negative cap = unlimited) drives everything. Free caps (50 garments / 20 outfits / 1 body photo) are enforced in the Wardrobe/Outfit create paths via `EntitlementService.EnsureCanAdd*`. AI credits live in the append-only `account_credit_ledger` (migration 012, all three stores): `CreditLedgerService` lazily grants a one-time free trial (8, top-up-to-config: accounts granted under an older smaller trial receive the difference on their next balance read via `GetCreditSumByReason`; lowering the config never claws back) and a once-per-UTC-month Premium allowance (100, rolls over — grants are stored without expiry; `expires_at` is reserved), debits on try-on confirm BEFORE queueing (insufficient balance → `ValidationException`), refunds failed jobs idempotently, never charges cache hits or `ClothesOnlyPreview`, and bypasses Admin entirely (unlimited). Tier gating rides `TryOnService.Estimate` (`ApplyEntitlementAvailability` → `RequiresUpgrade` on the estimate), so the confirm re-validation inherits it; Free is repriced to 1k via `ITryOnProvider.CapabilitiesFor(maxResolution)` (FASHN: `settings with { Resolution }`, different settings hash → tiers cache separately, and the tier resolution rides `TryOnGenerationSettings.Resolution` into the actual FASHN call). Premium/Admin jobs enqueue with `priority: true` (`ITryOnJobQueue.EnqueueAsync(jobId, priority, ct)`; Redis uses a second `:priority` list, popped first). Surfaces: `GET /api/account/entitlements`, `POST /api/admin/users/{id}/credits` (AdminAdjustment ledger rows; admin list shows a read-only raw balance, no lazy grants), estimate response carries `requiresUpgrade`/`creditBalance`/`creditsUnlimited`; Builder shows the credits chip + Premium pills + upgrade notice and disables confirm on insufficient balance (cache hits stay confirmable), Wardrobe's Add garment panel shows a cap-usage notice.
- Billing (paywall stage 4) is implemented to "insert the API key" readiness with Stripe: Application `BillingService` + `IBillingProvider` (Infrastructure `StripeBillingProvider` on Stripe.net 52 / `DisabledBillingProvider`), selected at startup via `Billing__Provider=Auto|Stripe|Disabled` (`Auto` = Stripe when `Stripe__SecretKey` is set, softly disabled otherwise — the app works unchanged without credentials). Config: `Stripe__SecretKey`/`Stripe__WebhookSecret`/`Stripe__PremiumMonthlyPriceId` (+`PremiumMonthlyDisplayPrice`)/`Stripe__TopUpPacks__N__{Id,Credits,PriceId,DisplayPrice}` (packs without a `PriceId` are not offered); success/cancel/portal URLs default to `Authentication__PublicOrigin` + `/upgrade?checkout=success|cancelled` and `/upgrade`. Endpoints: `GET /api/billing`, `POST /api/billing/checkout` (effective-Free only), `POST /api/billing/topup` (effective-Premium only), `POST /api/billing/portal` (requires a subscription row) — all session+CSRF; `POST /api/billing/webhook` is anonymous by design (allowlisted in `RequiresAuthenticatedUser`; the `Stripe-Signature` check is the auth), idempotent via the `billing_webhook_events` event-id gate, and handles `checkout.session.completed` + `customer.subscription.created/updated/deleted` (`invoice.paid` deliberately not handled). Webhooks upsert `billing_subscriptions` (migration 013, all three stores via `ISubscriptionRepository`/`IBillingEventRepository`) and flip stored roles Free↔Premium per Domain `BillingRules.GrantsPremium` (active/trialing/past_due; deleted → canceled → Free); pinned and stored-Admin roles are never rewritten; webhooks never grant subscription credits — the lazy monthly grant stays the single granting authority, while top-up payments append `TopUp` ledger rows from server-stamped session metadata (`CreditLedgerService.GrantTopUp`, sanity-bounded 1..10000). Admin list/detail carries read-only `subscriptionStatus`/`subscriptionPeriodEnd`. Frontend: `/upgrade` route (`UpgradePage` + `billing.css`: plan card, Stripe Checkout redirect, Premium top-up packs, `?checkout=success|cancelled` notices that invalidate session/entitlements/billing queries, graceful disabled state), Builder upgrade notice links to `/upgrade`, account settings shows a Plan row (See Premium link for Free, portal button for subscribed Premium), AdminPage gains a Subscription column; client functions live in `src/api/client.ts` (`getBillingStatus`/`startSubscriptionCheckout`/`startTopUpCheckout`/`openBillingPortal`).
...
- CI: `.github/workflows/ci.yml` runs backend tests → backend build → `npm ci` → `npm test` → `npm run build` strictly sequentially in ONE job (the frontend `pretest`/`prebuild` regenerate the OpenAPI client by building the API project — never parallelize), pinned to Node 24 (Node 26 breaks vitest jsdom localStorage); a second non-blocking job runs the Playwright e2e smoke against an in-memory http API + Vite dev server.
- Production Docker images must never contain `appsettings.json`/`.env`: the repo-root `.dockerignore` covers the frontend image (repo-root build context) and `outfit_planner_back/.dockerignore` (added 2026-08-10) covers the API image context. The production compose stack is configured via environment variables only. The untracked local `appsettings.json` (real FASHN/Google secrets, `TryOn.Provider=Fashn`) is a bare-`dotnet run` convenience; before the backend `.dockerignore` existed it leaked into the API image and silently switched the "production" stack to FASHN.
- Prod rehearsal passed 2026-08-10: the full `docker-compose.yml` stack was built and verified as a separate compose project (`docker compose -p <name>` with `FRONTEND_HTTP_PORT`/`FRONTEND_HTTPS_PORT` overridden so the live selfhost stack keeps running) — registration → wardrobe upload (Simple keyer, no rembg in prod compose) → outfit → mock try-on Succeeded → repeat confirm served from cache with no debit → anonymous share → signed URLs under `PUBLIC_ORIGIN` fetched through nginx (tampered signature → 404). Live FASHN renders stay blocked on FASHN account credits (provider returned 429 out-of-credits; the idempotent failure refund was verified live), so demos are mock-only for now.
...
$env:ConnectionStrings__Postgres = "Host=localhost;Port=15433;Database=outfit_planner;Username=outfit;Password=outfit;GSS Encryption Mode=Disable"
dotnet run --project outfit_planner_back\src\OutfitPlanner.Api\OutfitPlanner.Api.csproj
```
