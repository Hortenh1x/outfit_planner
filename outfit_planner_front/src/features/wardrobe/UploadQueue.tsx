...
export function UploadQueue({ items, onAcceptTag, onChangeItem, onRemove }: UploadQueueProps) {
  if (items.length === 0) {
      ...
      {items.map((item) => (
        <article className={item.status === 'invalid' ? 'upload-queue-row invalid' : 'upload-queue-row'} key={item.id}>
          <div className="upload-queue-heading">
            ...
            <button type="button" aria-label={`Remove ${item.file.name}`} onClick={() => onRemove(item.id)}>
              Remove
            </button>
          </div>
          <label>
          ...
          <div className="suggested-tags" aria-label={`Suggested tags for ${item.name}`}>
            {item.suggestedTags.map((tag) => (
              <button type="button" key={tag} onClick={() => onAcceptTag(item.id, tag)}>
                {tag}
              </button>
            ))}
          </div>
